...
Node <- R6Class("Node", list(
  ...
  initialize = function(id, depth = NULL, subset.idx, id.parent = NULL, child.type = NULL, objective.value.parent = NULL, objective.value = NULL, improvement.met, intImp) {
  ...
  computeChildren = function(X, Y, objective) {
    ...
    if (self$stop.criterion.met|self$improvement.met) {
      ...
      left.child = Node$new(id = 1, depth = self$depth + 1, subset.idx = idx.left, id.parent = self$id, child.type = "<=",  improvement.met = self$improvement.met, intImp = self$intImp)
      right.child = Node$new(id = 2, depth = self$depth + 1, subset.idx = idx.right, id.parent = self$id, child.type = ">",  improvement.met = self$improvement.met, intImp = self$intImp)
...
compute_tree = function(effect, testdata, objective = "SS_L2", n.split, impr.par = 0.1, min.split = 10) {
  #browser()
  if (objective == "SS_L1") {
  ...
  for (depth in seq_len(n.split)) {
    ...
    for (node.idx in seq_along(leaves)) {
      ...
      if (!is.null(node.to.split)) {
        node.to.split$computeSplit(X = input.data$X, Y = input.data$Y, objective = split.objective, impr.par = impr.par, optimizer = find_best_binary_split, min.split = min.split)
